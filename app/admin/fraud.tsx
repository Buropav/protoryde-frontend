...
export default function FraudReview() {
  ...
  const renderItem = ({ item }: { item: any }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.zoneText}>📍 {item.zone} | {item.trigger_type}</Text>

      <View style={styles.layerContainer}>
        {item.fraud_layers.map((layer: any, idx: number) => (
          <View key={idx} style={[styles.layerBadge, layer.passed ? styles.passed : styles.failed]}>
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No suspicious claims detected in this cycle.</Text>
        }
      />
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  layerBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4 },
  ...
  emptyText: { color: Colors.textMuted, textAlign: 'center', marginTop: 40, fontStyle: 'italic' }
});
