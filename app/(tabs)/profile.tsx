...
export default function ProfileScreen() {
  ...
  const { riderId, riderName, phoneNumber, policyId, setPolicyId } = useContext(RiderContext)!;
        ...
        { 
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert("Success", "Your coverage has been upgraded to ENHANCED!");
            } catch (err) {
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.header}>
          <View style={styles.avatarPlaceholder}>
             <Text style={styles.avatarText}>{riderName ? riderName.substring(0, 2).toUpperCase() : 'RY'}</Text>
          </View>
        ...
        <View style={[styles.card, styles.tierCard]}>
          ...
          <View style={styles.tierRow}>
            <Text style={[styles.tierValue, isEnhanced ? { color: Colors.primary } : { color: Colors.textPrimary }]}>
            ...
            {!isEnhanced && (
              <Pressable 
                ...
                disabled={upgrading}
              >
                {upgrading ? <ActivityIndicator size="small" color={Colors.background} /> : <Text style={styles.upgradeText}>UPGRADE</Text>}
              </Pressable>
            )}
          </View>
          <Text style={styles.tierLimit}>Payout Cap: ₹{policy?.coverage_cap || 2300}</Text>
        </View>
        ...
        <View style={styles.menuSection}>
          ...
          <Pressable style={styles.menuItem} onPress={() => router.push('/admin' as any)}>
            <Text style={[styles.menuText, { color: Colors.alert }]}>Switch to Admin Mode</Text>
            <Text style={styles.menuArrow}>›</Text>
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  avatarPlaceholder: { width: 64, height: 64, borderRadius: 32, backgroundColor: Colors.cardFill, justifyContent: 'center', alignItems: 'center', marginRight: 16, borderWidth: 1, borderColor: Colors.border },
  ...
  card: { backgroundColor: Colors.cardFill, padding: 20, borderRadius: 16, borderWidth: 1, borderColor: Colors.border, marginBottom: 24 },
  tierCard: { borderColor: Colors.primary },
  cardLabel: { fontSize: 12, color: Colors.textSecondary, marginBottom: 8, fontWeight: '600' },
  ...
  upgradeButton: { backgroundColor: Colors.primary, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8 },
