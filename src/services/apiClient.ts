...
function mapErrorCodeToMessage(
  ...
  if (
    ...
    fallbackMessage?.includes("UNSUPPORTED_ZONE")
  ) {
    return "This delivery zone is not yet supported.";
  }
  if (
    code === "UNSUPPORTED_TRIGGER" ||
    fallbackMessage?.includes("UNSUPPORTED_TRIGGER")
  ) {
    return "This trigger type is not recognized.";
  }
  if (
    code === "EXCLUSIONS_NOT_ACKNOWLEDGED" ||
    fallbackMessage?.includes("EXCLUSIONS_NOT_ACKNOWLEDGED")
  ) {
