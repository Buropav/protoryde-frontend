...
export const seedDemoRider = async (
  request: APIRequestContext,
  data: RiderSeed,
): Promise<void> => {
  const response = await request.post(
    "http://localhost:8000/api/demo/bootstrap",
    {
      data: {
...
export const simulateTriggerEvent = async (
  ...
  const response = await request.post(
    "http://localhost:8000/api/demo/simulate-trigger",
    {
      data: {
        rider_id: riderId,
...
export const completeOnboarding = async (
  ...
  await page
    .getByPlaceholder("XXXX XXXX 1234")
    .fill(opts.aadhaarLast4 ?? "1234");
  await page
    .getByPlaceholder("DD / MM / YYYY")
    .fill(opts.dob ?? "01 / 01 / 1998");
  await page.getByText("Continue").last().click();

  await expect(
    page.getByText("Tell us about your Delhivery partner profile."),
