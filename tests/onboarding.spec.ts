...
test.describe("Onboarding Flow", () => {
  test("enforces validation on key onboarding steps", async ({ page }) => {
    ...
    await page.getByText("Continue").last().click();
    await expect(
      page.getByText("Enter a valid UPI ID (example: yourname@upi)."),
    ).toBeVisible();
  });

  test("walks through onboarding screens with detailed UI checks", async ({
    page,
  }) => {
    await page.goto("/phone-verification");
    ...
    await expect(
      page.getByText("₹49.00", { exact: true }).first(),
    ).toBeVisible();
